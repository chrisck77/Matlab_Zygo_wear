...
%**************************************************************************
    ...
    case 4 
       ...
       if iDesLev==1;[Root] = uigetdir('C:\Data\DAF\','Select data folder'); 
        ...
        list_volum2 = NaN(size(tests,1)/2,15); %double array for writing to csv file
          ...
          list_volum2((i+1)/2,15) = max(vol_segm4) - min(vol_segm4);
          %list_volum2((i+1)/2,14) = max(vol_segm) - min(vol_segm);
        ...
        Headers_xls = {'Pump Number', 'PB material removed', 'PB alternative calc material removed',...
            ...
            'OMV max - min segment','OMV max-min movn avg segment 45','OMV max-min movn avg segment 22.5'};
        sheet =1;
